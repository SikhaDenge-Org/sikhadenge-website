...
import { startGmailMailboxWatch, syncGmailHistory } from "@/modules/email-automation/inbound/gmail-inbound-service";
...
async function main() {
  ...
  const watch = await startGmailMailboxWatch({ workspaceId: connection.workspaceId, connectionId: connection.id });
  const sync = await syncGmailHistory({ workspaceId: connection.workspaceId, connectionId: connection.id });
  process.stdout.write(`${JSON.stringify({ status: "PASS", account: allowedAccount, workspaceId: connection.workspaceId, connectionId: connection.id, watch: { historyId: watch.historyId, expiration: watch.expiration, persisted: watch.persisted }, sync: { discovered: sync.discovered, imported: sync.imported, replayed: sync.replayed, nextHistoryId: sync.nextHistoryId } }, null, 2)}\n`);
}
