...
rollback(){ code=$?; trap - ERR; cp -a "$BACKUP_DIR/.env.before" "$ENV_FILE" || true; pm2 restart "$PM2_PROCESS_NAME" --update-env >/dev/null 2>&1 || true; printf 'STATUS=ROLLED_BACK_AFTER_FAILURE\n' > "$BACKUP_DIR/rollback-result.txt" || true; exit "$code"; }
trap rollback ERR
node - "$ENV_FILE" <<'NODE'
