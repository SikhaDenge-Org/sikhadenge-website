...
RUN_ID="email-e8-reconcile-$(date -u +%Y%m%dT%H%M%SZ)"
...
rollback() {
  ...
  cleanup
  printf 'EMAIL_E8_RECONCILE_ROLLBACK=true exit_code=%s\n' "$rc"
  exit "$rc"
}
...
printf 'RECONCILE_TRACKED_DIRTY_BEGIN\n%s\nRECONCILE_TRACKED_DIRTY_END\n' "$actual_dirty"
...
printf 'RECONCILE_RUNTIME_PRECHECK=DRY_RUN_EXTERNAL_WRITES_OFF\n'
...
printf 'RECONCILE_PRE_PM2_PID=%s\nRECONCILE_PRE_PM2_STATUS=%s\nRECONCILE_PRE_PM2_CWD=%s\n' "$pid" "$status" "$cwd"
test -n "$pid"
test "$status" = online
RUNTIME_APP="${cwd:-$APP}"
...
printf 'RECONCILE_PRE_APP_BUILD_ID=%s\nRECONCILE_PRE_RUNTIME_BUILD_ID=%s\n' "$old_app_build" "$old_runtime_build"
...
expected_diff='apps/whatsapp-agent-dashboard/scripts/email-e8-live-reconcile.sh'
printf 'RECONCILE_TARGET_DIFF_BEGIN\n%s\nRECONCILE_TARGET_DIFF_END\n' "$actual_diff"
...
printf 'RECONCILE_REVIEWED_RUNTIME_BLOBS=PASS\n'
...
test -n "$new_build"
printf 'RECONCILE_STAGED_BUILD_ID=%s\n' "$new_build"

set -a
source "$APP/.env"
set +a
npx tsx scripts/email-provider-e8-readiness.ts | tee "$BACKUP/e8-readiness-pre.txt"
grep -q '^E8_PROVIDER_READINESS=PASS$' "$BACKUP/e8-readiness-pre.txt"
grep -q '^E8_EXTERNAL_EMAIL_SENT=false$' "$BACKUP/e8-readiness-pre.txt"
printf 'RECONCILE_PRE_READINESS=PASS\n'
...
printf 'RECONCILE_POST_PM2_PID=%s\nRECONCILE_POST_PM2_STATUS=%s\nRECONCILE_POST_PM2_CWD=%s\n' "$post_pid" "$post_status" "$post_cwd"
test -n "$post_pid"
...
printf 'RECONCILE_POST_SHA=%s\n' "$(git rev-parse HEAD)"
printf 'RECONCILE_POST_APP_BUILD_ID=%s\n' "$(cat "$APP/.next/BUILD_ID")"
printf 'RECONCILE_POST_RUNTIME_BUILD_ID=%s\n' "$(cat "$RUNTIME_APP/.next/BUILD_ID")"
printf 'RECONCILE_LOGIN_LOCAL_HTTP=%s\nRECONCILE_LOGIN_PUBLIC_HTTP=%s\n' "$login_local" "$login_public"
printf 'RECONCILE_RUNTIME_POSTCHECK=DRY_RUN_EXTERNAL_WRITES_OFF\n'
printf 'E8_EXTERNAL_EMAIL_SENT=false\n'
printf 'EMAIL_E8_RECONCILE=PASS\n'
