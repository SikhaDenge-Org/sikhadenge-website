...
printf 'SEQUENCE=provision,preflight,activate,first-run,verify\nEXTERNAL_WRITES=false\nRUNTIME_MODE=DRY_RUN\n'
if [[ "$APPLY" != "1" ]]; then printf 'INFO: preview only; no env, PM2, or systemd mutation performed\n'; exit 0; fi
[[ "$(id -u)" == "0" ]] || fail "APPLY=1 requires root"
install -d -m 700 "$BACKUP_DIR"
rollback(){ code=$?; trap - ERR; APPLY=1 SERVICE_NAME="$SERVICE_NAME" bash scripts/email-automation-scheduler-deactivate.sh >/dev/null 2>&1 || true; if [[ -f "$BACKUP_DIR/.env.before" ]]; then APPLY=1 LIVE_APP="$LIVE_APP" ENV_FILE="$ENV_FILE" BACKUP_DIR="$BACKUP_DIR" bash scripts/email-automation-dryrun-rollback.sh >/dev/null 2>&1 || true; fi; printf 'STATUS=ROLLED_BACK_AFTER_FAILURE\nFAILED_EXIT_CODE=%s\n' "$code" > "$BACKUP_DIR/activation-result.txt" || true; chmod 600 "$BACKUP_DIR/activation-result.txt" 2>/dev/null || true; exit "$code"; }
trap rollback ERR
