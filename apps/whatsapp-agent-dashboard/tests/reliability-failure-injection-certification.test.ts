...
assert.match(drill, /grep -q 'P1001'/);
assert.match(drill, /grep -q '127\.0\.0\.1:1'/);
assert.match(drill, /PHASE20_DB_FAILURE_OUTPUT_SHA256/);
assert.match(workflow, /environment: whatsapp-agent-production/);
assert.match(workflow, /StrictHostKeyChecking=yes/);
assert.match(workflow, /retention-days: 30/);
assert.match(workflow, /if: always\(\)/);
assert.match(workflow, /phase20-reliability-evidence\.txt 2>&1/);
