...
const DEFINITIONS: readonly LifecycleDefinition[] = [
  ...
  },
  {
    ...
    body: "Hi {{contactName}}, you started a SikhaDenge registration but did not finish it. If you still want to continue, our team can help with course, batch, admission, or payment questions. If you no longer want registration reminders, you can unsubscribe below.",
    cta: "Continue registration support",
    purpose: "MARKETING",
  },
  {
    key: "payment-pending",
...
export function buildLifecycleEmailDocument(item: LifecycleDefinition): EmailTemplateDocument {
  const variables = [
    { key: "contactName", label: "Contact name", required: false, fallback: "Learner" },
    ...(item.purpose === "MARKETING" ? [{ key: "unsubscribe_url", label: "Unsubscribe URL", required: true }] : []),
  ];
  return {
    ...
    blocks: [
      ...
      ...(item.purpose === "MARKETING" ? [{ id: `${item.key}-unsubscribe`, type: "BUTTON" as const, label: "Unsubscribe from registration reminders", url: "{{unsubscribe_url}}" }] : []),
  ...
  for (const item of DEFINITIONS) {
    ...
    if (!summary) {
      const detail = await runtime.service.create({
        ...
        document: buildLifecycleEmailDocument(item),
  ...
  for (const item of DEFINITIONS) {
    ...
    const created = await createAutomationFlow({
      ...
      nodes: [
        ...
        { id: `${item.key}-email`, kind: "ACTION", type: "SEND_EMAIL", label: "Send lifecycle email", config: { templateId: template.id, templateVersionId: template.versionId, emailPurpose: item.purpose } },
