...
import { assertManualEmailDispatchPolicy, internalRecipientAllowlist } from "./manual-send-policy";
...
export class ManualEmailSendService {
  ...
  list(workspaceId: string, take = 50) {
    return prisma.engageEmailMessage.findMany({ where: { workspaceId }, orderBy: { createdAt: "desc" }, take: Math.min(Math.max(take, 1), 100), select: {
      id: true, templateId: true, templateVersionId: true, senderIdentityId: true, senderResolutionSource: true, status: true, runtimeMode: true,
