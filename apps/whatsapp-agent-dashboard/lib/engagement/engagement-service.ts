...
export async function updateAppointment(input: {
  ...
  await prisma.$transaction(async (tx) => {
    ...
    if (whatsappWorkspaceId && status !== "SCHEDULED") {
      await supersedePendingWhatsAppAutomationEvents(tx, {
        workspaceId: whatsappWorkspaceId,
        trigger: "APPOINTMENT_REMINDER",
        sourceEventIdPrefix: `engagement-appointment-reminder:${appointment.id}:`,
      });
...
export async function createPaymentRecord(input: {
  ...
  await prisma.$transaction(async (tx) => {
    ...
    if (emailWorkspaceId && payment.contactId && (status === "PENDING" || status === "PAID")) {
      if(status === "PAID") await supersedePendingEmailAutomationEvents(tx,{workspaceId:emailWorkspaceId,trigger:"PAYMENT_ABANDONED",contactId:payment.contactId,sourceEventIdPrefix:`engagement-payment-abandon:${payment.id}`});
    ...
    if (whatsappWorkspaceId && payment.contactId && (status === "PENDING" || status === "PAID")) {
      if(status === "PAID") await supersedePendingWhatsAppAutomationEvents(tx,{workspaceId:whatsappWorkspaceId,trigger:"PAYMENT_ABANDONED",contactId:payment.contactId,sourceEventIdPrefix:`engagement-payment-abandon:${payment.id}`});
