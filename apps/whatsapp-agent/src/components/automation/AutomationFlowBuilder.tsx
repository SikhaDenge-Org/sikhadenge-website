...
function configField(type: string) {
  ...
  if (type === "SEND_TEMPLATE") return { key: "templateId", label: "Approved WhatsApp template ID", placeholder: "Template record ID" };
  if (type === "SEND_EMAIL") return { key: "emailTemplateId", label: "Approved email template ID", placeholder: "Email template record ID" };
  ...
  if (type === "EMAIL_RECEIVED" || type === "EMAIL_REPLIED" || type === "EMAIL_BOUNCED") return { key: "connectionId", label: "Email connection ID", placeholder: "Optional provider connection ID" };
  if (type === "FORM_SUBMITTED") return { key: "formId", label: "Form ID", placeholder: "Optional form ID" };
  if (type === "PAYMENT_PENDING" || type === "PAYMENT_PAID") return { key: "provider", label: "Payment provider", placeholder: "Optional provider filter" };
  if (type === "APPOINTMENT_CREATED" || type === "APPOINTMENT_REMINDER") return { key: "appointmentType", label: "Appointment type", placeholder: "Demo / counselling" };
  return null;
}
  ...
  async function simulate() {
    ...
    try {
      const response = await fetch(`/api/automation/flows/${encodeURIComponent(selectedId)}/simulate`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ sample: { message: "Demo class details", stage: "NEW", tag: "Website Lead", email: "student@example.com" } }),
      });
      ...
      setSuccess("Dry-run simulation completed. No external message was sent.");
      await loadFlows(selectedId);
    } catch (reason) {
  ...
  return (
    <div className="automation-builder">
      <section className="automation-metrics">
        ...
        <article><span>External actions</span><strong>{runtime?.externalActionsReady ? "Ready" : "Locked"}</strong><small>WhatsApp and email sends require explicit cutover.</small></article>
      </section>
      ...
      <div className="automation-workspace">
        ...
        <section className="automation-editor">
          ...
          <div className="automation-details-grid">
            <label><span>Flow name</span><input value={name} maxLength={120} onChange={(event) => setName(event.target.value)} placeholder="New lead welcome email" /></label>
            <label><span>Description</span><input value={description} maxLength={500} onChange={(event) => setDescription(event.target.value)} placeholder="Send approved welcome email when a lead is created" /></label>
          </div>
          ...
          <footer className="automation-editor-footer">
            <div className="automation-actions">
              <button type="button" onClick={() => void saveFlow()} disabled={busy === "save"}>{busy === "save" ? "Saving…" : selectedId ? "Save changes" : "Save draft"}</button>
              <button type="button" className="secondary" onClick={() => void simulate()} disabled={!selectedId || busy === "simulate"}>{busy === "simulate" ? "Simulating…" : "Simulate"}</button>
            </div>
            {selectedId ? (
              <div className="automation-status-actions">
                <button type="button" className="secondary" onClick={() => void changeStatus("ACTIVE")} disabled={busy.startsWith("status:")}>Activate definition</button>
                <button type="button" className="secondary" onClick={() => void changeStatus("PAUSED")} disabled={busy.startsWith("status:")}>Pause</button>
                <button type="button" className="secondary" onClick={() => void changeStatus("ARCHIVED")} disabled={busy.startsWith("status:")}>Archive</button>
              </div>
            ) : null}
          </footer>

          {validation ? (
            <section className={`automation-validation ${validation.valid ? "valid" : "invalid"}`}>
              <strong>{validation.valid ? "Flow definition valid" : "Flow definition needs changes"}</strong>
              <p>{validation.triggerCount} trigger · {validation.actionCount} actions</p>
              {validation.errors.map((message) => <p key={message} className="error">{message}</p>)}
              {validation.warnings.map((message) => <p key={message} className="warning">{message}</p>)}
            </section>
          ) : null}

          {simulation.length > 0 ? (
            <section className="automation-simulation">
              <strong>Dry-run result</strong>
              {simulation.map((line) => <p key={line}>{line}</p>)}
            </section>
          ) : null}
        </section>
      </div>
